...
test("managed-auth Seatbelt launch owns persistent CODEX_HOME without a candidate workspace", async () => {
  ...
  try {
    ...
    const proxy = new URL(String(captured?.env?.HTTPS_PROXY));
    assert.equal(proxy.protocol, "http:");
    assert.equal(proxy.hostname, "127.0.0.1");
    assert.ok(proxy.port);
    assert.ok(proxy.username && proxy.password, "the loopback proxy requires a per-session secret");
    assert.equal(captured?.env?.HTTP_PROXY, captured?.env?.HTTPS_PROXY);
    assert.equal(captured?.env?.http_proxy, captured?.env?.HTTPS_PROXY);
    assert.equal(captured?.env?.https_proxy, captured?.env?.HTTPS_PROXY);
    assert.equal(captured?.env?.NO_PROXY, "");
    assert.equal(captured?.env?.no_proxy, "");
    assert.equal(captured?.argv[0], path.join(root, "sandbox-exec"));
    assert.match(String(captured?.argv[2]), /\(deny network\*\)/);
    assert.ok(String(captured?.argv[2]).includes(`(allow network-outbound (remote tcp "localhost:${proxy.port}"))`));
