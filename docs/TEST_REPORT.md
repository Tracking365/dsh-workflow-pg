...
| `npm test` | passed: 54 tests, 0 failures/skips | Original/domain/SQLite/Git-process fixtures, an explicit content-locked native `pagination-v1` lifecycle, real Cordis/ToolRuntime and DSH AgentLoop fixtures, candidate-bound DSH session, provider subprocess-seam/cancellation fixtures, declared-permission guard, plus injected HTTP tests. |
...
| Live Codex safety boundaries + independent model | partial/blocked | One narrow, explicit current-login fixture proves real provider startup, cwd handoff, teardown, and a constrained write; an in-memory provider-wire fixture proves published-turn cancellation semantics. Enforceable sandboxing, network and credential boundaries, actual-App-Server cancellation, independent reviewer behavior, and a deployed live task remain unverified. |
...
| C04–C08 | partial/blocked | Snapshot mutation, strict live block, candidate-workspace parent composition and official-provider cwd seam tested; one real provider process observed the candidate cwd and exact allowed proof write. Native code also refuses declared full-access, unreadable, or unknown provider modes. OS isolation, network and real credential boundaries are not implemented. |
...
| F01–F02 | partial | macOS process/cooperative cancellation, native fixture terminal cancellation, offline AgentLoop stream cancellation/preserved text, bridge and candidate-parent disposal proof, official-provider in-memory published-turn interrupt/termination/wait-for-exit, candidate-parent create failure, and unproven stop retaining the lease are tested; escaped hostile descendants and real-App-Server cancellation require sandbox/live evidence. |
...
54/54 does not mean the 56 acceptance IDs all pass; the one manual provider invocation is not included in that count. M0–M3 are explicitly incomplete.
