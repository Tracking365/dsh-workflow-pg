...
| Authorized one-shot live Codex fixture (manual, non-CI) | passed: one invocation | A real official App Server ran through `DshCandidateWorkspaceCodexExecutor` with the current login, `permissionMode: "never"`, and `env: {}`. The controller and candidate were separate temporary directories. The observed subprocess started exactly once with the canonical candidate cwd; it completed, left `README.md` unchanged, created no unexpected top-level files, and wrote the exact allowed proof file. No full-access mode, remote, push, merge, or deployment was configured. This does not prove OS containment, network-egress control, credential non-disclosure, or actual-provider cancellation. |
| Live Codex safety boundaries + independent model | partial/blocked | One narrow, explicit current-login fixture now proves real provider startup, cwd handoff, teardown, and a constrained write. Enforceable sandboxing, network and credential boundaries, cancellation, independent reviewer behavior, and a deployed live task remain unverified. |
...
| C04–C08 | partial/blocked | Snapshot mutation, strict live block, candidate-workspace parent composition and official-provider cwd seam tested; one real provider process observed the candidate cwd and exact allowed proof write. OS isolation, network and real credential boundaries are not implemented. |
...
| H01–H03 | partial/fail-closed | No implicit model invocation/fallback; a registered provider still cannot bypass disabled mode or the workspace-binding guard. One separately authorized fixture invoked the real provider with `permissionMode: "never"` and a temporary cwd, but does not establish a general live safety boundary. |

51/51 does not mean the 56 acceptance IDs all pass; the one manual provider invocation is not included in that count. M0–M3 are explicitly incomplete.
