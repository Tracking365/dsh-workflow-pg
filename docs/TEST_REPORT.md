...
Linux x64; Node v22.16.0; npm 10.9.2; Git 2.47.3. Network access from the shell was
unavailable. No DSH/Codex binary or authorized model configuration was present.

Build/typecheck used preinstalled TypeScript **5.8.3**, with preinstalled @types/node
**25.1.0** supplied through a local uncommitted symlink. Repository pins remain 5.7.2 and
22.10.2. A fresh exact-lock `npm ci` is **blocked/unverified**, not passed. Lock integrity
values for remaining dependencies came from the original repository; no dependency
installation or npm publication is claimed.

## Executed

| Command/layer | Result | What it proves |
|---|---|---|
| `npm run check` | passed | Strict TypeScript check with the above actual compiler/types. |
| `npm test` | passed: 35 tests, 0 failures/skips | Original 4 tests plus domain, SQLite, real Git/process fixture and injected HTTP tests. |
| `npm run demo` | passed | Real failing baseline -> fixed candidate -> real TAP pass -> fixture review -> patch/report, awaiting human acceptance. |
...
| C09 | partial | Shell-free argv/env test on Linux; Windows not tested. |
...
| F01–F02 | partial | Linux process/cooperative cancellation, unproven stop retains lease; escaped hostile descendants require sandbox. |
...
| G07–G08 | partial | Bounds/redaction/env and honest Linux-only matrix; regex is not complete secret detection. |
| H01–H03 | blocked/fail-closed | No implicit model invocation/fallback; no live evidence. |

35/35 does not mean the 56 acceptance IDs all pass. M0–M3 are explicitly incomplete.
